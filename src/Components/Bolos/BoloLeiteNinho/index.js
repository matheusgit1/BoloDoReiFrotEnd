...
export default function BoloLeiteNinho(){

    const [dataImage, setDataImage] = useState([]);
    const [loading, setLoading] = useState(false);
    const [promotion,setPromotion] = useState(false);
    const [price, setPrice] = useState(GlobalVars.ValorBoloLeiteNinho);

    useEffect(()=>{
        setLoading(true);
        async function getCakeImagesFromStorage(){
        await firestore().collection('bolo_leite_ninho').onSnapshot(snapshot => {
            const listImage = [];
            snapshot.forEach(doc => {
              ...
              if(doc.id == 'preco'){
                ...
                GlobalVars.ValorBoloLeiteNinho = parseFloat(price).toFixed(2);
                console.log(GlobalVars.ValorBoloLeiteNinho);
              }
          ...
          <View>
            <View style={styles.container}>
              <View style={styles.Cab}>
                <Text style={styles.Title}>Bolo de Leite Ninho</Text>
              ...
              <Buttons promo={promotion} screenNavigation="Bolo de Leite Ninho"/>
            </View>
          </View>
        </ScrollView>
      </ScrollView>
